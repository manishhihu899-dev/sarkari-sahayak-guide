...
import { 
  FileText, CheckCircle, AlertTriangle, Globe, Building,
  Bookmark, XCircle, HelpCircle, Info, Sparkles, ExternalLink,
} from "lucide-react";
  ...
  return (
    <div className="min-h-screen bg-background pb-32">
      ...
      <div className="relative overflow-hidden wave-border">
        <div className="absolute inset-0 bg-animated-gradient" style={{ background: `linear-gradient(135deg, hsl(${themeColor}) 0%, hsl(${themeColor} / 0.8) 50%, hsl(${themeColor} / 0.6) 100%)`, backgroundSize: '400% 400%' }} />
        ...
        <div className="relative z-10 p-6 pt-6 pb-8">
          <div className="flex items-center gap-2 mb-3 opacity-0 animate-fade-up" style={{ animationDelay: '100ms', animationFillMode: 'forwards' }}>
            ...
            <span className="text-xs font-semibold text-white/90 uppercase tracking-wider">Sarkari Sahayak</span>
          </div>
          <h1 className="text-2xl font-bold text-white mb-2 leading-tight opacity-0 animate-fade-up" style={{ animationDelay: '150ms', animationFillMode: 'forwards' }}>{subService.titleHi}</h1>
          <p className="text-sm text-white/80 leading-relaxed opacity-0 animate-fade-up" style={{ animationDelay: '200ms', animationFillMode: 'forwards' }}>{subService.description}</p>
        </div>
      </div>
      ...
      <main className="px-4 py-6 space-y-6">
        ...
        {!isScheme && (
          <div className="bg-card rounded-xl p-4 shadow-card animate-fade-up" style={{ animationDelay: "50ms" }}>
            ...
            <p className="text-xs text-muted-foreground mt-2">{completedSteps.length} / {subService.steps.length} steps done - Tap step to mark complete</p>
          </div>
        )}

        {subService.eligibility && subService.eligibility.length > 0 && (
          <InfoSection icon={CheckCircle} title="Kaun Apply Kar Sakta Hai" items={subService.eligibility} variant="success" delay={100} />
        )}
          ...
          <div className="space-y-1">
            ...
            {subService.steps.map((step, index) => (
              <StepCard key={step.step} step={step.step} title={step.title} description={step.description}
                isCompleted={completedSteps.includes(step.step)} onToggle={() => toggleStep(step.step)}
                delay={250 + index * 50} totalSteps={subService.steps.length} />
            ))}
          </div>
        )}
        ...
        <div className="bg-card rounded-xl p-4 shadow-card space-y-3 animate-fade-up" style={{ animationDelay: "400ms" }}>
          <h3 className="font-semibold text-foreground flex items-center gap-2">
            <Building className="w-5 h-5 text-primary" /> Official Information
          </h3>
          <div className="space-y-2 text-sm">
            ...
            <div className="flex items-start gap-2">
              ...
              <div><span className="text-muted-foreground">Office: </span><span className="text-foreground">{subService.officeName}</span></div>
            </div>
          </div>
        </div>
        ...
        {subService.commonMistakes && subService.commonMistakes.length > 0 && (
          <InfoSection icon={XCircle} title="❌ Galtiyan jo nahi karni" items={subService.commonMistakes} variant="error" delay={500} />
        )}

        {subService.problemReasons && subService.problemReasons.length > 0 && (
          <InfoSection icon={HelpCircle} title="⚠️ Exact problem ka reason" items={subService.problemReasons} delay={550} />
        )}
        ...
        <div className="flex gap-3">
          ...
          <Button variant="accent" className="flex-1" onClick={() => window.open(subService.officialWebsite, "_blank")}>
            <Globe className="w-5 h-5" /> Official Site
          </Button>
        </div>
